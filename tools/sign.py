...
usage: sign.py [--place doctype|charset] [--no-console] [--zip] PATH [PATH...]
...
def sign_text(src, place='charset', console=True):
    changed = False
    ...
    with zipfile.ZipFile(zp) as zin, zipfile.ZipFile(buf, 'w', zipfile.ZIP_DEFLATED) as zout:
        for item in zin.infolist():
            ...
            if item.filename.lower().endswith(('.html','.htm')) and not item.filename.startswith('__MACOSX'):
                try:
                    txt = data.decode('utf-8'); new, ch = sign_text(txt, place, console)
...
def main(argv):
    place, console, do_zip, paths = 'charset', True, False, []
    it = iter(argv)
    for a in it:
    ...
    for root in paths:
        ...
        for p in files:
            if p.suffix.lower() == '.zip':
                if do_zip: n = sign_zip(p, place, console); stats[f'zip:{p.name}'] = n
                continue
            r = sign_file(p, place, console); stats[r] = stats.get(r, 0) + 1
